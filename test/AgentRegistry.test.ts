...
describe("AgentRegistry", function () {
  ...
  describe("registerAgent", function () {
    it("owner can register an agent", async function () {
      await expect(
        ...
        .withArgs(agentWallet, "agent1.agentry.eth", "claude-opus-4-7", "coding");
    ...
    it("reverts on duplicate wallet", async function () {
      await registry.registerAgent("agent1.agentry.eth", agentWallet, "m", "s", "u");
      await expect(
        registry.registerAgent("agent2.agentry.eth", agentWallet, "m", "s", "u")
    ...
    it("reverts on duplicate ENS name", async function () {
      ...
      await registry.registerAgent("agent1.agentry.eth", agentWallet, "m", "s", "u");
      await expect(
        registry.registerAgent("agent1.agentry.eth", wallet2, "m", "s", "u")
  ...
  describe("resolveENS", function () {
    it("resolves registered ENS name to wallet", async function () {
      await registry.registerAgent("agent1.agentry.eth", agentWallet, "m", "s", "u");
      expect(await registry.resolveENS("agent1.agentry.eth")).to.equal(agentWallet);
    });
  ...
  describe("getAgentByENS", function () {
    it("returns agent data for known ENS", async function () {
      await registry.registerAgent("agent1.agentry.eth", agentWallet, "claude-opus-4-7", "coding", "uri");
      const agent = await registry.getAgentByENS("agent1.agentry.eth");
  ...
  describe("updateMetadata", function () {
    it("owner can update metadata URI", async function () {
      await registry.registerAgent("agent1.agentry.eth", agentWallet, "m", "s", "old-uri");
